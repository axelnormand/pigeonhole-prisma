...
export const PunbbUserModelBase = ModelBase
  ...
  .props({
    ...
    tokens: types.union(types.undefined, types.null, types.string),
  })
  .views(self => ({
    get store() {
...
export class PunbbUserModelSelector extends QueryBuilder {
  ...
  get tokens() { return this.__attr(`tokens`) }
}
export function selectFromPunbbUser() {
  return new PunbbUserModelSelector()
}

export const punbbUserModelPrimitives = selectFromPunbbUser().username.signature.registered.last_visit.last_post.tokens
